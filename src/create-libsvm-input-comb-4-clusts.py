...
''' Call with params:
    ...
    - four filename containing the clusters (as output by the divisive clustering script),
      for features A, B, C, and D
    - an arbitrary number of gzipped files divisible by 4, each of which contains
...
def main(labelfilename, clustfnameA, clustfnameB, clustfnameC, clustfnameD, *repofnames):
    clustsA = read_clusters(clustfnameA)
    clustsB = read_clusters(clustfnameB)
    clustsC = read_clusters(clustfnameC)
    clustsD = read_clusters(clustfnameD)
    repo_to_lab_map = get_repo_to_lab_map(labelfilename)
    lab_to_num_map = construct_lab_to_num_map(set(repo_to_lab_map.values()))

    fnamesA = repofnames[:(len(repofnames)/4)]
    fnamesB = repofnames[(len(repofnames)/4):(2 * len(repofnames)/4)]
    fnamesC = repofnames[(2 * len(repofnames)/4):(3 * len(repofnames)/4)]
    fnamesD = repofnames[(3 * len(repofnames)/4):]

    assert len(fnamesA) == len(fnamesB)
    assert len(fnamesA) == len(fnamesC)
    assert len(fnamesA) == len(fnamesD)

    for fnameA, fnameB, fnameC, fnameD in zip(fnamesA, fnamesB, fnamesC, fnamesD):
        ...
        reponame4 = os.path.basename(fnameD).replace('.gz', '')
        assert reponame1 == reponame2
        assert reponame1 == reponame3
        assert reponame1 == reponame4
        lab = repo_to_lab_map[reponame1]
        sys.stderr.write('extracting features from %s...\n' % reponame1)
        print construct_libsvm_line(lab_to_num_map[lab], fnameA, fnameB, fnameC, fnameD, clustsA, clustsB, clustsC, clustsD)
...
def construct_libsvm_line(labelnum, fnameA, fnameB, fnameC, fnameD, clustsA, clustsB, clustsC, clustsD):
    ...
    featvalsD = get_feat_vals_from_repo(gzip.open(fnameD, 'rb'), clustsD)
    lenA = len(featvalsA)
    lenB = len(featvalsB)
    lenC = len(featvalsC)
    return '%d %s %s %s %s' % (labelnum,
                              ' '.join(['%d:%f' % (i,v) for i,v in enumerate(featvalsA)]),
                              ' '.join(['%d:%f' % ((i + lenA), v) for i,v in enumerate(featvalsB)]),
                              ' '.join(['%d:%f' % ((i + lenA + lenB), v) for i,v in enumerate(featvalsC)]),
                              ' '.join(['%d:%f' % ((i + lenA + lenB + lenC), v) for i,v in enumerate(featvalsD)]))
