...
''' Call with params:
    ...
    - two filename containing the clusters (as output by the divisive clustering script),
      for features A and B
    - an arbitrary even number of gzipped files, each of which contains
      the newline-delimited tokenized repo (with delimiters between files).
      All of the files for features A must occur before all the fiels for features B.
      Further, the feature A files must occur in the same order (and be the same) as the
      feature B files.

      so it'll be something like
      ./create-libsvm-input-comb-clusts.py lab-file clustA clustB repo1.A.tok.gz repo2.A.tok.gz repo1.B.tok.gz repo2.B.tok.gz
'''
...
def main(labelfilename, clustfnameA, clustfnameB, *repofnames):
    ...
    fnamesA = repofnames[:(len(repofnames)/2)]
    fnamesB = repofnames[(len(repofnames)/2):]
    assert len(fnamesA) == len(fnamesB)

    for fnameA, fnameB in zip(fnamesA, fnamesB):
        ...
        print construct_libsvm_line(lab_to_num_map[lab], fnameA, fnameB, clustsA, clustsB)
...
def construct_libsvm_line(labelnum, fnameA, fnameB, clustsA, clustsB):
